...
public class CartController {
	...
	public ModelAndView cart(Principal principal){
	 ...
	 public ModelAndView deleteCart(@PathVariable(value="id") int id,Principal principal ){
		 ...
		 System.out.println("------------------------------old quantity reached"+oldQuantity+"-------------------------");
	...
	// mapping for contact us page
		...
		public ModelAndView cart(@PathVariable("id") int cartId){
			ModelAndView mv = new ModelAndView("index");
			mv.addObject("title", "Cart");
			mv.addObject("userClickCart", true);
			mv.addObject("cartItem",cartItemDao.userlist());
			return mv;
		}
		
		
		 @RequestMapping(value = "/add/{id}", method = RequestMethod.PUT)
		 public ModelAndView addItem (@PathVariable(value="id") int id, @AuthenticationPrincipal User activeUser){
			 ModelAndView mv = new ModelAndView("index");
			 UserDto userDto = userDao.getUserDetail(id);
			 Cart cart = userDto.getCart();
			 ServicesDto servicesDto = servicesDao.get(id);
			 List<CartItem> cartItems = cart.getCartItems();
			 for (int i=0; i < cartItems.size(); i++){
				 if(servicesDto.getId() == cartItemDao.getUserDetail(id).getServicesDto().getId()){
		      ...
		      /*  List<CartItem> cartItems = cart.getCartItems();
		        ...
		        for (int i=0; i < cartItems.size(); i++){
		            if(servicesDto.getId() == cartItem.get(i).getServicesDto().getId()){
		                CartItem cartItem = cartItems.get(i);
		                cartItem.setQuantity(cartItem.getQuantity() + 1);
				...
				public ModelAndView addCart(@PathVariable("userId")int userId,@PathVariable("id")int id){
					ModelAndView mv = new ModelAndView("index");
					mv.addObject("title", "Cart");
					mv.addObject("userClickCart", true);
	...
	/*	
		...
		public ModelAndView addcart() {
			...
			UserDto userDto = userDao.getUserDetail(userId);		
			Cart cart = userDto.getCart();
	cartItemDao		ServicesDto servicesDto = servicesDao.;
			cartItem.setCart(cart);
			cartItem.setServicesDto(servicesDto);
			cartItem.setQuantity(1);
			cartItem.setTotalPrice(servicesDto.getPrice() * cartItem.getQuantity());
